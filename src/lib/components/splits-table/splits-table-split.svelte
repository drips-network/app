...
>
  <!-- vector group -->
  <div
    class="relative h-8 {isIncoming ? 'flex items-end justify-end' : ''} {isFaded
  ...
  >
    ...
    {#if typeof split.percent === 'string'}
      <div
        class="absolute overlay flex items-center justify-center {isIncoming ? 'pr-1' : 'pr-2px'}"
      >
        <!-- lozenge centered-->
  ...
  <div class="flex-1 flex" class:justify-end={isIncoming}>
    {#if typeof split.subject === 'string'}
      <div class="typo-text" style="color: var(--color-foreground)">
        {@html split.subject}
      </div>
