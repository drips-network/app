<script lang="ts">
  ...
  type KycLike = { reviewAnswer?: string | null } | null | undefined;
  type KybLike =
    | {
        hasKyb?: boolean;
        authorized?: boolean;
  ...
  interface Props {
    ...
    kybStatus?: KybLike;
    /**
     * When true, hides the "verify your identity" link that points to the
     * authenticated settings page (which a magic-link user can't reach).
     */
    hideIdentityLinks?: boolean;
  }

  let { grant, actionContext, kycStatus, kybStatus, hideIdentityLinks = false }: Props = $props();
  ...
  let kybData = $derived(
    hasKyb && kybAuthorized && kybStatus?.stellarAddress
      ? {
          ...
          memoType: kybStatus.memoType ?? null,
          memoValue: kybStatus.memoValue ?? null,
        }
      : undefined,
  );

  function openTestTransactionFlow(g: GrantDetailDto) {
    modal.show(Stepper, undefined, testTransactionFlow(g, kybData, actionContext));
  }
  ...
  function openWithdrawalFlow(g: GrantDetailDto) {
    ...
    const prefill =
      !kybData && lastTest
        ...
        : undefined;
    modal.show(Stepper, undefined, withdrawalFlow(g, prefill, kybData, actionContext));
  }
  ...
  async function handleCancelWithdrawal() {
    await doWithConfirmationModal('Are you sure you want to cancel this pending withdrawal?', () =>
      doWithErrorModal(async () => {
        ...
        try {
          await actionContext.cancelWithdrawal();
          await invalidate(actionContext.invalidateKey);
        } finally {
      ...
      {:else if hasKyb && kybAuthorized && !kycApproved}
        <div class="kyc-warning">
          <AnnotationBox type="info">
            <span>
              You are authorized to withdraw on behalf of your organization via entity verification
              (KYB). Personal identity verification is not required.
            </span>
          </AnnotationBox>
        </div>
      {/if}
        ...
        {#if grant.isOrgGrant}
          <div class="overview-row">
            ...
            <StatusBadge size="small" color="primary" icon={Orgs}>
              ...
              >
                Org grant
                <InfoCircle style="width: 1rem; height: 1rem; fill: currentColor; cursor: help;" />
              </span>
            </StatusBadge>
          </div>
          <div class="overview-row">
  ...
  <div class="right-pane">
    <Section
      ...
      skeleton={{
        ...
        emptyStateText: 'Request a test transaction or withdrawal to see your transaction history.',
      }}
    >
