...
describe('DripListService', () => {
  ...
  describe('getByOwnerAddress', () => {
    ...
    it.skip('should throw when the user has more than one DripList', async () => {
      // Arrange
      const owner = '0x123';
      const ownerNftSubAccounts: NftSubAccount[] = [
        {
      ...
      const originalGetDripListProjects = sut['_getDripListProjects'];

      dripListService['_getDripListProjects'] = vi
        .fn(sut['_getDripListProjects'])
        .mockResolvedValueOnce([
          {
            weight: nftSubAccountMetadata[0].data.projects[0].weight,
            project: {
      ...
      sut['_getDripListProjects'] = originalGetDripListProjects;
    });
  });
