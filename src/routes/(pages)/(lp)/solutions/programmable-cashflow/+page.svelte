...
<SolutionHeader>
  ...
  <svelte:fragment slot="headline"
    >Achieve complex funding flows with the Drips Protocol</svelte:fragment
  >
  <svelte:fragment slot="description">
    Web3 payments should be flexible, not stuck in old, rigid structures. Drips enables flexible and
    extensible streaming and splitting of any ERC-20, to any Ethereum address or off-chain
    identities.
  </svelte:fragment>
</SolutionHeader>
...
<SolutionCard>
  ...
  <svelte:fragment slot="description">
    Send a single stream to ten thousand Ethereum addresses at once? Fund fifty GitHub users with a
    single transaction? No problem.
  </svelte:fragment>
  <svelte:fragment slot="line-items">
    <SolutionLineItem icon={TokenStreams}>
      Stream funds over time instead of sending lump sums
    </SolutionLineItem>
    <SolutionLineItem icon={User}>
      Send funds to off-chain identities such as GitHub repositories, ORCIDs, and more
    </SolutionLineItem>
    <SolutionLineItem icon={Splits}>
      Nest different split lists, even if they’re owned by different people
    </SolutionLineItem>
  </svelte:fragment>
</SolutionCard>
...
<SolutionInterstitial>
  <svelte:fragment slot="headline">Achieve complex automated funding flows on-chain</svelte:fragment
  >
  <svelte:fragment slot="description">Talk to us and tell us about your use-case.</svelte:fragment>
</SolutionInterstitial>

<SolutionCard
  illustrationPadding="1rem"
  button={{ text: 'Learn more', href: 'https://docs.drips.network/the-protocol/overview' }}
>
  <svelte:fragment slot="illustration">
    <CodeBox
      height="20rem"
      width="30rem"
      code={`/// @notice A Drips driver implementing repository-based account identification.
