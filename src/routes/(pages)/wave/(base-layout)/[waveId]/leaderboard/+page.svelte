...
<div class="page">
    ...
    <div class="filter-control">
      <Dropdown
        options={[
          ...
          { value: 'current-cycle', title: 'Current Cycle' },
        ]}
        value={data.currentCycleOnly ? 'current-cycle' : 'all-time'}
        onchange={(val) => {
          if (val === 'current-cycle') {
            goto(page.url.pathname + '?filter=current-cycle');
          } else {
  ...
  <span class="typo-text intro" style:color="var(--color-foreground-level-5)"
    >Showing {data.leaderboard.pagination.total} user{moreThanOne ? 's' : ''} who {moreThanOne
      ? 'have'
      : 'has'} earned points in the {data.wave.name} Wave{data.currentCycleOnly
      ? "'s current Cycle"
      : ''}. At the end of each Wave Cycle, rewards are distributed to each contributor according to
    the share of the total points earned during that cycle.
  </span>
