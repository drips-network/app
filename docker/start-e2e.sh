...
if [ $PROD_BUILD = true ]; then
  ...
  # TODO: revert
  docker compose build && APP_USE_LOCAL_TESTNET_WALLET_STORE=true GRAPHQL_API_TAG=remove-auth-ipfs-and-make-source-non-nullable EVENT_PROCESSOR_TAG=ecosystems ECOSYSTEMS_TAG=main docker compose -f docker-compose.yml up --renew-anon-volumes --detach
fi
