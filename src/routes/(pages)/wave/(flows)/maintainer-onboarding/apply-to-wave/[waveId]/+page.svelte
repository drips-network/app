<script lang="ts">
  ...
  import { applyRepoToWave } from '$lib/utils/wave/waves';
  import FlowStepWrapper from '../../../shared/flow-step-wrapper.svelte';

  let { data } = $props();

  function checkAlreadyApplied(repoId: string) {
    return data.ownWaveRepos.data.some(
      (waveRepo) => waveRepo.repo.id === repoId && waveRepo.waveId === data.wave.id,
    );
  }
  ...
  async function handleApply() {
    ...
    const result = await doBatchWithErrorModal(
      selected.map((repoId) => async () => {
        try {
          await applyRepoToWave(undefined, data.wave.id, repoId);
        } catch (e) {
          return {
            success: false,
    ...
    if (result.some((res) => !res.success)) {
      ...
      goto(`/wave/maintainer-onboarding/apply-to-wave/${data.wave.id}/success`);
    }
  }
</script>
...
<FlowStepWrapper
  ...
  description="Choose which repos you’d like to apply to {data.wave
    .name}. They must match the criteria set for the Wave in order to be eligible."
>
  ...
  {#snippet leftActions()}
    <Button icon={ArrowLeft} href="/wave/maintainer-onboarding/apply-to-wave">Choose Wave</Button>
  {/snippet}

  <AnnotationBox type="info">
    After you apply, we'll email you when your repos have been accepted into {data.wave.name}. You
    can always check on the status of your applications on the maintainer dashboard.
  </AnnotationBox>
